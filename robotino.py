...
def follow_path(planner: dict,
    ...
    print(f" Параметры: max_speed={max_speed} м/с, kp={kp}, точность={goal_tolerance} см")
    ...
    try:
        while True:
            ...
            odom = get_odometry()
            if odom is None:
                print(" Не удалось получить одометрию")
            ...
            if not success and step_count % 20 == 0:
                print(" Проблема с отправкой команды")
        ...
        print("\n Движение остановлено пользователем")
        stop_robot()
        return False
    except Exception as e:
        print(f" Ошибка: {e}")
